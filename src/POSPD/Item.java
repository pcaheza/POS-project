...
/**
 * Item is something that is sold in the store. The system shall provide a way
...
public class Item implements Comparable<Item> {
	...
	public Price getPriceForDate(LocalDate date) {
		Price price = null;
		for (Price prc : prices) {
			if (date.equals(prc.getEffectiveDate())) {
				return prc;
			}
		}
		return price;
	...
	public BigDecimal calcAmountForDateQty(LocalDate date, int quantity) {
		Price priceForDate = getPriceForDate(date);
		return priceForDate.calcAmountForQyt(quantity);
	}

	@Override
	public String toString() {
		
		return "\n number=" + number + ", description=" + description + ", upcs=" + upcs.keySet() + ", price=" + getPriceForDate(LocalDate.now()).getPrice()
	...
	/**
	 * this.number="3" and other item.number="2" return 1 this.number="2" and other
	 * item.number="3" return -1 this.number="3" and other item.number="3" return 0
	 */
	@Override
	public int compareTo(Item other) {
		return this.number.compareToIgnoreCase(other.getNumber());
	}
